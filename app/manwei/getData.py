...
# def getData():
#     for i in range(0, 291, 15):
#         url = "https://www.douban.com/tag/%E6%BC%AB%E5%A8%81/movie?start={}&filter=".format(i)
...
def getData():
    for i in range(0, 290, 15):
        url = "https://www.douban.com/tag/%E6%BC%AB%E5%A8%81/movie?start={}".format(i)
        r = re.get(url, headers=head, timeout = 100)
        content = r.content
        rt = str(content, "utf-8")
        soup = bs(rt, "html.parser")
        try:
            ...
            tags = soup.find_all('div', class_='desc')
            links = soup.find_all('a', class_='title')

            for tit, tag, lin in zip(titles, tags, links):
                title = tit.get_text().split('\n')
                title=title[0]

                tags = tag.get_text().strip()

                link = str(lin.get('href'))

                savenews(title,tags,link)
        except:
            print("get-error")


def savenews(title, tags, link):
    img, summary = getSummary(link)
    ...
    try:
        cursor.execute("INSERT INTO douban(title,tags,link,img,summary) values(%s,%s,%s,%s,%s)",
                       (title, tags, link, img, summary))
        print("ok")
        db.commit()
    except:
        print("db-error")
...
def getSummary(link):
    try:
        ...
        rt = str(content, "utf-8")
        soup = bs(rt, 'html.parser')
        summarys = soup.find('div', id='link-report').find_all('span')
        summary = summarys[0].get_text().strip().replace(" ", "")
        pic = soup.find('div', id='mainpic').find_all('img')
        img = str(pic[0].get('src'))
